...
class App:
    '''The application, where most things are implemented'''
    ...
    def __initLt(self):
        ...
        self.lable1.place(
            x=0, y=0, height=self.scrset['ptop'], width=self.scrset['w']/2)
    ...
    def __initRt(self):
        ...
        self.lable2.place(
            x=self.scrset['w']/2, y=0, height=self.scrset['ptop'], width=self.scrset['w']/2)
            ...
            try:
                ...
                if j >= self.scrset['w'] - self.scrset['pside']:
                    if h + self.scrset['buttonH'] > self.scrset['h']:
                        msgbox.showwarning(
                            self.__name, "The button may be out of the UI")
                    j = self.scrset['pside']
                ...
                msgbox.showerror(
                    self.__name, f"KeyError raised: {e}, maybe you wrote an invalid button id in settings.json")
            except Exception as e:
                msgbox.showerror(
                    self.__name, f"A(n) {e.__class__.__name__} raised when placing the buttons: {e}")
    ...
    def loadFromFile(self, file=None, filetr=None):
        ...
        if file is None:
            file = self.setting['i']['file']
        if filetr is None:
            filetr = self.setting['o']['file']
        try:
            ...
            msgbox.showerror(
                self.__name, f"A(n) {e.__class__.__name__} Exception raised: {e}")
            return -1
        self.fr.delete('1.0', END)
        f.close()
        self.lfr = self.text.split('\n')
    ...
    def loadset(self, file):
        ...
        try:
            ...
            self.btnwid = (
                rawwid + self.scrset['pmid'])/self.scrset['buttonpline'] - self.scrset['pmid']
        except FileNotFoundError as e:
            msgbox.showerror(self.__name, "Settings.json Missing!")
        except Exception as e:
            msgbox.showerror(
                self.__name, f"A(n) {e.__class__.__name__} raised when loading the settings: {e}")
    ...
    def moveto(self, index, save=True):
        ...
        try:
            if index >= self.end:
                msgbox.showerror(
                    self.__name, "That's the end of your passage!")
                return -1
            elif index < 0:
                msgbox.showerror(
                    self.__name, "That's the begining of your passage!")
                return -1
    ...
    def jump(self, save=True):
        ...
        i = self.require4int(
            self.__name, "Input the line that\nyou want to jump to")
    ...
    def trans(self):
        ...
        try:
            ...
            msgbox.showerror(
                self.__name, f'A(n) {e.__class__.__name__} raised when translating: {e}')
